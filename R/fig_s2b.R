...
fig_s2b <- function(tables, save =FALSE, ...){
  ...
  plot <- ggplot2::ggplot(cov_full) +
    ...
    ggplot2::geom_hline(yintercept = -0.33, col = "black", linetype = 2) +
    ggplot2::scale_x_continuous("Age at sampling [days]") +
    ggplot2::scale_y_continuous("Social rank at sampling") +
    ggplot2::scale_shape_manual("Social class", labels = c("High-ranking", "Low-ranking"), values=c(3, 1)) +
